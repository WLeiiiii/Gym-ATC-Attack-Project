...
class EpsilonScheduler:
    ...
    def get_eps(self, epoch, step):
        if self.schedule_type == "smoothed":
            return self.smooth_schedule(epoch * self.num_steps_per_epoch + step, self.init_step, self.final_step,
                                        self.init_value, self.final_value, self.mid_point, self.beta)
        else:
            return self.linear_schedule(epoch * self.num_steps_per_epoch + step, self.init_step, self.final_step,
...
def main():
    ...
    epsilon_scheduler = EpsilonScheduler("linear", schedule_start,
                                         schedule_start + schedule_length - 1, starting_epsilon, end_epsilon, 1)
    max_eps = end_epsilon

    agent = Agent(state_size=env.observation_space.shape[0], action_size=env.action_space.n)
    load_path = '../save_model/dqn_random_goal_model_02.pth'
    load_frame = 1
